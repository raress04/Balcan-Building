...
async function backupFile(filePath, backupDir) {
    ...
    try {
        ...
        console.log(`  ⏭️  Backup already exists: ${relativePath}`);
    } catch {
    ...
    for (const entry of entries) {
        ...
        if (entry.isDirectory() && entry.name !== 'backup') {
            ...
            if (['.jpg', '.jpeg', '.png'].includes(ext)) {
                ...
                if (result.success) {
                    console.log(`  ✅ ${entry.name}: ${(result.originalSize / 1024).toFixed(0)}KB → ${(result.newSize / 1024).toFixed(0)}KB (-${result.savings}%)`);
                    totalOriginal += result.originalSize;
                    totalNew += result.newSize;
                    processed++;
                } else if (result.skipped) {
                    console.log(`  ⏭️  ${entry.name}: ${result.reason}`);
...
async function main() {
    ...
    try {
        ...
        console.log('⚠️  Portfolio directory not found\n');
    }
    ...
    try {
        ...
        if (heroResult.success) {
            console.log(`  ✅ hero-construction.jpg: ${(heroResult.originalSize / 1024).toFixed(0)}KB → ${(heroResult.newSize / 1024).toFixed(0)}KB (-${heroResult.savings}%)`);
            grandTotalOriginal += heroResult.originalSize;
            grandTotalNew += heroResult.newSize;
        } else if (heroResult.skipped) {
            console.log(`  ⏭️  hero-construction.jpg: ${heroResult.reason}`);
        }
        console.log('');
    } catch {
    ...
    const totalSavings = ((grandTotalOriginal - grandTotalNew) / grandTotalOriginal * 100).toFixed(1);
